(()=>{
  ...
  async function loadUsers(){if(!ownerAdminRoles.has(currentUser?.role)||!$('#adminUsers'))return;try{const rows=await api('/admin/users');$('#adminUsers').innerHTML=rows.map(u=>`<div class="item"><div class="row between wrap"><div><b>${esc(u.display_name)}</b><div class="muted">${esc(u.username)} · ${esc(u.role)}</div></div><span class="pill">${u.active?'Active':'Disabled'}</span></div><div class="row wrap"><label class="inline-admin-role">Role <select data-admin-role="${u.id}"><option value="technician" ${u.role==='technician'?'selected':''}>Technician</option><option value="delivery" ${u.role==='delivery'?'selected':''}>Delivery</option><option value="sales" ${u.role==='sales'?'selected':''}>Salesperson</option><option value="staff" ${u.role==='staff'?'selected':''}>Staff</option><option value="service_manager" ${u.role==='service_manager'?'selected':''}>Service Manager</option><option value="manager" ${u.role==='manager'?'selected':''}>Manager</option><option value="admin" ${u.role==='admin'?'selected':''}>Admin</option><option value="owner" ${u.role==='owner'?'selected':''}>Owner</option></select></label><button class="secondary" data-admin-toggle="${u.id}" data-active="${u.active?'1':'0'}">${u.active?'Disable':'Enable'}</button><button class="secondary" data-admin-password="${u.id}">Reset Password</button></div></div>`).join('')||'<p class="muted">No staff accounts.</p>';$$('[data-admin-role]').forEach(s=>s.onchange=async()=>{try{await api(`/admin/users/${s.dataset.adminRole}`,{method:'PATCH',body:JSON.stringify({role:s.value})});loadUsers()}catch(e){alert(e.message)}});$$('[data-admin-toggle]').forEach(b=>b.onclick=async()=>{try{await api(`/admin/users/${b.dataset.adminToggle}`,{method:'PATCH',body:JSON.stringify({active:b.dataset.active!=='1'})});loadUsers()}catch(e){alert(e.message)}});$$('[data-admin-password]').forEach(b=>b.onclick=async()=>{const p=prompt('Enter a new temporary password (10+ characters):','');if(!p)return;try{await api(`/admin/users/${b.dataset.adminPassword}/password`,{method:'PATCH',body:JSON.stringify({password:p})});alert('Password updated.')}catch(e){alert(e.message)}})}catch(e){$('#adminUsers').innerHTML=`<p class="error">${esc(e.message)}</p>`}}
