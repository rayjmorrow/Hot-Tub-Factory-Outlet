(()=>{
  ...
  function ensureAdminUi(user){
    ...
    if(!$('#admin')){
      const s=document.createElement('section');s.id='admin';s.className='view';s.hidden=true;s.innerHTML=`<div class="row between wrap"><div><h1>Service Admin</h1><p class="muted">Staff accounts, roles, and recent system history.</p></div><button id="adminAddUser">+ Staff Account</button></div><div class="ops-detail-grid"><div class="card"><h2>Staff Accounts</h2><div id="adminUsers" class="list"></div></div><div class="card"><div class="row between wrap"><div><h2>Recent Audit History</h2><p class="muted">Customer, equipment, work order, invoice, payment, warranty and payment-method changes.</p></div><button class="secondary" id="adminRefreshAudit">Refresh</button></div><div id="adminAudit" class="list"></div></div></div>`;$('main').appendChild(s);$('#adminAddUser').onclick=openNewUser;$('#adminRefreshAudit').onclick=loadAudit;
    }
  }
  ...
  function openNewUser(){$('#adminUserForm').reset();d.showModal()}
  $('#adminSaveUser').onclick=async e=>{e.preventDefault();try{const b=Object.fromEntries(new FormData($('#adminUserForm')));await api('/admin/users',{method:'POST',body:JSON.stringify(b)});d.close();await loadUsers()}catch(err){alert(err.message)}};

  async function loadUsers(){try{const rows=await api('/admin/users');$('#adminUsers').innerHTML=rows.map(u=>`<div class="item"><div class="row between wrap"><div><b>${esc(u.display_name)}</b><div class="muted">${esc(u.username)} · ${esc(u.role)}</div></div><span class="pill">${u.active?'Active':'Disabled'}</span></div><div class="row wrap"><button class="secondary" data-admin-toggle="${u.id}" data-active="${u.active?'1':'0'}">${u.active?'Disable':'Enable'}</button><button class="secondary" data-admin-password="${u.id}">Reset Password</button></div></div>`).join('')||'<p class="muted">No staff accounts.</p>';$$('[data-admin-toggle]').forEach(b=>b.onclick=async()=>{try{await api(`/admin/users/${b.dataset.adminToggle}`,{method:'PATCH',body:JSON.stringify({active:b.dataset.active!=='1'})});loadUsers()}catch(e){alert(e.message)}});$$('[data-admin-password]').forEach(b=>b.onclick=async()=>{const p=prompt('Enter a new temporary password (10+ characters):','');if(!p)return;try{await api(`/admin/users/${b.dataset.adminPassword}/password`,{method:'PATCH',body:JSON.stringify({password:p})});alert('Password updated.')}catch(e){alert(e.message)}})}catch(e){$('#adminUsers').innerHTML=`<p class="error">${esc(e.message)}</p>`}}

  async function loadAudit(){try{const rows=await api('/admin/audit?limit=100');$('#adminAudit').innerHTML=rows.map(a=>{const record=a.new_data||a.old_data||{};const label=record.work_order_number||record.invoice_number||record.first_name&&record.last_name?`${record.first_name||''} ${record.last_name||''}`.trim():record.description||`Record ${a.record_id||''}`;return `<div class="item"><div class="row between wrap"><b>${esc(a.action)} · ${esc(a.table_name.replace(/^service_/,''))}</b><span class="muted">${dt(a.created_at)}</span></div><div>${esc(label||`Record ${a.record_id||''}`)}</div></div>`}).join('')||'<p class="muted">No audit history yet.</p>'}catch(e){$('#adminAudit').innerHTML=`<p class="error">${esc(e.message)}</p>`}}
