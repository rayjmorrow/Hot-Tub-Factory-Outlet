(()=>{
  ...
  style.textContent=`.customer-orders{margin:1rem 0}.customer-orders .order-card{border:1px solid #dfe5ec;border-radius:10px;padding:.8rem;background:#fff;margin:.65rem 0}.customer-orders .order-item{display:grid;grid-template-columns:1fr auto;gap:.5rem;padding:.45rem 0;border-bottom:1px solid #edf1f5}.customer-orders .scope{font-size:.76rem;font-weight:800}.customer-orders .scope.recurring{color:#166534}.customer-orders .scope.one{color:#475467}.customer-orders .change-row{border-left:3px solid #d0d5dd;padding:.3rem 0 .3rem .65rem;margin:.4rem 0}.customer-orders .perm-note{background:#f8fafc;border:1px solid #dfe5ec;border-radius:8px;padding:.55rem .7rem;margin:.55rem 0}`;
  ...
  async function renderOrders(customerId){
    ...
    panel.innerHTML=`<div class="row between wrap"><div><h3 style="margin:.1rem 0">Sales Orders & Auto-Ship</h3><div class="muted">Scheduled online orders, past shipments and recurring shipment settings.</div></div><div class="row wrap"><button class="primary" id="newSaleOrder">+ New Sales Order</button>${canFullOps()?'<button class="secondary" id="newDiscountCode">+ Discount Code</button><button class="secondary" id="newAutoship">+ Auto-Ship</button>':''}</div></div>
    ...
    $('#ordersUpcoming').innerHTML=upcoming.map(o=>orderCard(o,x.recurringOrders||[])).join('')||'<p class="muted">No open sales orders.</p>';
    $('#ordersPast').innerHTML=past.map(o=>orderCard(o,x.recurringOrders||[],true)).join('')||'<p class="muted">No past sales orders yet.</p>';
  ...
  function orderCard(o,subs,readOnly=false){
    ...
    const balance=Math.max(0,Number(o.total_amount||0)-Number(o.amount_paid||0));
    const ready=balance<.005||o.delivery_exception_authorized===true;
    const payState=balance<.005?'<span class="pill" style="background:#dcfce7;color:#166534">PAID IN FULL</span>':`<span class="pill" style="background:#fff7ed;color:#9a3412">${moneySafe(balance)} BALANCE</span>`;
    const exception=o.delivery_exception_authorized?`<div class="muted"><b>Delivery exception:</b> ${esc(o.delivery_exception_reason||'Authorized')} · ${esc(o.delivery_exception_by||'Manager')}</div>`:'';
    return `<div class="order-card"><div class="row between wrap"><div><b>${esc(o.order_number||`Order #${o.id}`)}</b><div>${o.scheduled_ship_date?`Ships ${dateSafe(o.scheduled_ship_date)}`:'No delivery date'} · <span class="pill">${esc(o.status)}</span> ${payState}</div></div><b>${moneySafe(o.total_amount)}</b></div>
      <div class="muted">Paid ${moneySafe(o.amount_paid||0)} · Balance ${moneySafe(balance)}</div>
      ${exception}
      ${linked?`<div class="muted">Generated from auto-ship · every ${linked.frequency_value} ${esc(linked.frequency_unit)}</div>`:''}
      <div>${(o.items||[]).map(i=>`<div class="order-item"><div><b>${Number(i.quantity)} × ${esc(i.description)}</b><div class="muted">${esc(i.sku||'No SKU')} · ${moneySafe(i.unit_price)} each · <span class="scope ${i.item_scope==='recurring'?'recurring':'one'}">${i.item_scope==='recurring'?'Recurring':'This shipment only'}</span></div></div><div>${moneySafe(i.line_total)}${readOnly?'':`<br><button class="secondary" style="padding:.3rem .5rem" data-edit-order-item="${i.id}" data-order-id="${o.id}">Edit Qty</button>`}</div></div>`).join('')||'<p class="muted">No items.</p>'}</div>
      ${readOnly?'':`<div class="row wrap" style="margin-top:.7rem">
        <button class="primary" data-add-order-item="${o.id}">+ Add Product</button>
        <button class="secondary" data-discount-code="${o.id}">Apply Discount Code</button>
        ${balance>.005?`<button class="secondary" data-payment-link="${o.id}">Create Balance Payment Link</button>`:''}
        ${canFullOps()&&balance>.005&&!o.delivery_exception_authorized?`<button class="secondary" data-delivery-exception="${o.id}">Authorize Delivery Exception</button>`:''}
        ${canFullOps()?`<button class="${ready?'primary':'secondary'}" data-schedule-delivery="${o.id}" ${ready?'':'disabled title="Paid in full or Rick/Ray exception required"'}>Schedule Delivery${ready?'':' - Payment Required'}</button><button class="secondary" data-edit-order="${o.id}">Edit Shipment</button>`:''}
      </div>`}</div>`;
  }
  ...
  function bindOrderButtons(customerId,x){
    ...
    $$('[data-payment-link]').forEach(b=>b.onclick=async()=>{try{const x=await api(`/orders/${b.dataset.paymentLink}/payment-link`,{method:'POST'});if(navigator.clipboard)await navigator.clipboard.writeText(x.url);prompt(`Balance payment link for ${x.customer_name||'customer'} — ${moneySafe(x.balance)} due.\n\nCopy/send this link by text or email:`,x.url)}catch(e){alert(e.message)}});
    $$('[data-schedule-delivery]').forEach(b=>b.onclick=async()=>{const o=(x.orders||[]).find(v=>String(v.id)===String(b.dataset.scheduleDelivery));if(!o)return;const balance=Math.max(0,Number(o.total_amount||0)-Number(o.amount_paid||0));if(balance>.005&&!o.delivery_exception_authorized)return alert('Delivery is locked until the spa is paid in full or Rick/Ray authorizes an exception.');const date=prompt('Delivery date (YYYY-MM-DD):','');if(!date)return;const time=prompt('Delivery time (HH:MM):','09:00')||'09:00';try{const scheduled=new Date(`${date}T${time}:00`).toISOString();await api('/work-orders',{method:'POST',body:JSON.stringify({customer_id:customerId,customer_order_id:o.id,assigned_to:'Delivery',assigned_team:'Delivery',job_type:'delivery',scheduled_start:scheduled,complaint:`Spa delivery — ${o.order_number||('Order '+o.id)}`,status:'scheduled'})});alert('Delivery scheduled. It is now on the Delivery calendar.');await loadCustomer(customerId)}catch(e){alert(e.message)}});
    $$('[data-delivery-exception]').forEach(b=>b.onclick=async()=>{const reason=prompt('Reason for delivery payment exception (example: Cash due on delivery):','Cash due on delivery');if(!reason)return;try{await api(`/orders/${b.dataset.deliveryException}/delivery-exception`,{method:'POST',body:JSON.stringify({authorized:true,reason})});await loadCustomer(customerId)}catch(e){alert(e.message)}});
    ...
    if(canFullOps()){
      ...
      if($('#newDiscountCode'))$('#newDiscountCode').onclick=async()=>{const code=String(prompt('New discount code:','')||'').trim().toUpperCase();if(!code)return;const kind=confirm('OK = percentage discount\nCancel = fixed dollar discount')?'percent':'fixed';const value=Math.max(0,Number(prompt(kind==='percent'?'Discount percent:':'Discount dollars:','10')||0));const description=prompt('Description / reason:','Approved customer discount')||'';const minimum=Math.max(0,Number(prompt('Minimum order subtotal:','0')||0));try{await api('/order-discount-codes',{method:'POST',body:JSON.stringify({code,description,discount_type:kind,discount_value:value,minimum_subtotal:minimum})});alert(`Discount code ${code} created.`)}catch(e){alert(e.message)}};
    }
  }
})();