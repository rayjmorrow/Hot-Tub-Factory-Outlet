(()=>{
  ...
  loadCustomer=async function(id){await originalLoadCustomer(id);try{await renderValueHistory(id)}catch(e){console.warn('Customer value history unavailable',e)}};
  ...
  async function renderValueHistory(customerId){
    ...
    panel.innerHTML=`<div><h3 style="margin:.1rem 0">Online Sales & Service History</h3><div class="relationship ${relClass}">${esc(s.relationship||'Customer')}</div><div class="muted">Current scope: online purchases and service activity. In-store POS sales remain separate for now.</div></div>
      <div class="value-stats">
        <div class="value-stat"><span class="muted">Lifetime Online Sales</span><b>${moneySafe(s.lifetime_retail)}</b></div>
        <div class="value-stat"><span class="muted">Lifetime Service</span><b>${moneySafe(s.lifetime_service)}</b></div>
        <div class="value-stat"><span class="muted">Online Chemicals · 12 mo</span><b>${moneySafe(s.chemical_12_months)}</b></div>
        <div class="value-stat"><span class="muted">Tracked Relationship</span><b>${moneySafe(s.lifetime_total)}</b></div>
        ...
        <div class="value-stat"><span class="muted">Last Online Purchase</span><b>${dateSafe(s.last_retail_purchase)}</b></div>
        <div class="value-stat"><span class="muted">Last Online Chemical Purchase</span><b>${dateSafe(s.last_chemical_purchase)}</b></div>
      </div>
      <div class="value-tabs"><button class="value-tab active" data-value-tab="timeline">Timeline</button><button class="value-tab" data-value-tab="purchases">Online Purchase History</button><button class="value-tab" data-value-tab="service">Service History</button></div>
    ...
    $('#valuePurchases').innerHTML=(x.retail||[]).map(r=>`<div class="item"><div class="row between wrap"><div><b>${dateSafe(r.transaction_date)} · ${esc(r.description)}</b><div class="muted">${esc([r.category,r.brand,r.model,r.transaction_number].filter(Boolean).join(' · '))}</div></div><b>${moneySafe(r.total_amount)}</b></div></div>`).join('')||'<p class="muted">No online purchases synced yet.</p>';
