...
router.post('/orders/:id/payment-link',auth,async(req,res)=>{
  ...
  const base=process.env.SERVICE_ORDER_PAYMENT_URL||'https://htfo-service-operations.onrender.com/order-payment.html';
...
router.post('/public/order-payment/session',async(req,res)=>{
  try{
    ...
    const returnUrl=(process.env.SERVICE_ORDER_PAYMENT_URL||'https://htfo-service-operations.onrender.com/order-payment.html')+'?complete=1';
...
router.post('/public/authorize-order-webhook',(req,res)=>{
  const signatureOk=verifyAuthorizeWebhook(req);
  const eventType=String(req.body?.eventType||'');
  const transId=String(req.body?.payload?.id||'').trim();

  // Authorize.Net requires an immediate HTTP 200 acknowledgement.
  // Validation and reconciliation continue after the response.
  res.status(200).json({ok:true});
  console.log(`Authorize.Net webhook received: event=${eventType||'unknown'} transaction=${transId?'present':'missing'} signature=${signatureOk?'valid':'invalid'}`);

  if(!signatureOk){
    console.warn('Authorize.Net webhook ignored after acknowledgement: signature validation failed');
    return;
  }
  if(!transId||!/authcapture|payment/i.test(eventType))return;

  setImmediate(async()=>{
    try{
      const j=await anet({getTransactionDetailsRequest:{merchantAuthentication:merchantAuth(),transId}});
      const t=j.transaction||{};
      if(String(t.responseCode||'')!=='1')return;
      ...
      console.error('Authorize.Net webhook reconciliation failed:',e?.message||e);
    }
  });
});
