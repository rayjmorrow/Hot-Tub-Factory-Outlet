...
export async function initServicePaymentGuard(){
  await q(`
    ...
    CREATE OR REPLACE FUNCTION htfo_require_payment_before_delivery_schedule() RETURNS trigger AS $$
    ...
    BEGIN
      IF COALESCE(NEW.job_type,'service')='delivery' AND NEW.scheduled_start IS NOT NULL THEN
        ...
        IF COALESCE(order_paid,0) + 0.005 < COALESCE(order_total,0)
           AND COALESCE(exception_ok,false)=false THEN
    ...
    CREATE TRIGGER trg_htfo_require_payment_before_delivery_schedule
      ...
      ON service_work_orders
      FOR EACH ROW EXECUTE FUNCTION htfo_require_payment_before_delivery_schedule();
  `);
}
