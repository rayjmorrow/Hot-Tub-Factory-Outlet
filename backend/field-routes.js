...
function canManage(req){
  return ['admin','owner','manager','service_manager'].includes(String(req.user?.role||'').toLowerCase());
}
...
router.get('/field/work-orders/:id',auth,async(req,res)=>{
  const w=await q(`SELECT w.*,concat_ws(' ',c.first_name,c.last_name) customer_name,c.phone,c.email,c.street,c.street2,c.city,c.state,c.zip,e.equipment_type,e.brand,e.model,e.serial_number
...
router.patch('/field/deliveries/:id/progress',auth,async(req,res)=>{
  ...
  if(!canAccessWorkOrder(req,w))return res.status(403).json({error:'This delivery is not assigned to you'});
...
router.post('/field/deliveries/:id/complete',auth,async(req,res)=>{
  ...
  const updated=(await q(`UPDATE service_work_orders
    SET status='completed',completed_at=NOW(),delivery_completed_by=$2,customer_signature=delivery_signature_data,updated_at=NOW()
    WHERE id=$1 RETURNING *`,[req.params.id,actor])).rows[0];
