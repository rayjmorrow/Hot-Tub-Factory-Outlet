...
app.get('/health',(req,res)=>res.json({ok:true,taxjar:process.env.TAXJAR_SANDBOX==='true'?'sandbox':'production',authorize:process.env.AUTHORIZE_SANDBOX==='true'?'sandbox':'production',authorizeConfigured:Boolean(authLoginId&&authTransactionKey),ghl:Boolean(process.env.GHL_API_TOKEN&&process.env.GHL_LOCATION_ID)}));
