(()=>{
  ...
  async function renderPaymentPanel(customerId){
    ...
    $$('[data-charge-invoice]').forEach(b=>b.onclick=async()=>{
      const amount=Number(b.dataset.balance||0);
      if(!confirm(`Charge the card on file ${money(amount)} for this invoice?`))return;
      try{const x=await api(`/invoices/${b.dataset.chargeInvoice}/charge-card-on-file`,{method:'POST',body:JSON.stringify({amount})});alert(`Approved. Transaction ${x.transaction_id}. Payment recorded.`);await loadCustomer(customerId)}catch(e){alert(e.message)}
    });
  }
})();