...
function showApp(){$('#login').hidden=true;$('#app').hidden=false;$('#who').textContent=user?.name||user?.username||'';const deliveryUser=String(user?.role||'').toLowerCase()==='delivery'||/delivery/i.test(String(user?.name||user?.username||''));if(deliveryUser){const h=$('#jobsView h1');if(h)h.textContent='Delivery Loop';const w=$('#weekView h1');if(w)w.textContent='Upcoming Deliveries';}$('[data-manager-only]').forEach(x=>x.hidden=!canManageCalendar());$('#todayLabel').textContent=new Date().toLocaleDateString('en-US',{weekday:'long',month:'long',day:'numeric'});$('#showAll').checked=canManageCalendar();const showAllWrap=$('#showAll')?.closest?.('label');if(showAllWrap)showAllWrap.hidden=!canManageCalendar();$('#showAll').disabled=!canManageCalendar();$('#weekSubtitle').textContent=canManageCalendar()?'Full dispatch view. Tap a job to open it.':'Your assigned calls for the next 7 days.';$('#calendarSubtitle').textContent=canManageCalendar()?'Full dispatch calendar — tap a day to schedule, edit, or reassign.':'Your assigned calendar — tap a day, then tap an appointment to open it.';showFieldView('jobsView')}
...
$('#logout').onclick=logout;$('#backJobs').onclick=()=>showFieldView('jobsView');$('#showAll').onchange=()=>{if(!canManageCalendar()){$('#showAll').checked=false}loadJobs()};
async function loadJobs(){const rows=await api('/work-orders');const today=localYmd();let items=rows.filter(w=>w.scheduled_start&&localYmd(new Date(w.scheduled_start))===today);if(!canManageCalendar()||!$('#showAll').checked)items=mine(items);items.sort((a,b)=>new Date(a.scheduled_start)-new Date(b.scheduled_start));$('#jobs').innerHTML=items.map(w=>`<article class="job"><div class="row"><span class="pill">${esc(w.status)}</span><span class="muted">${esc(w.assigned_to||'Unassigned')}</span></div><h3>${time(w.scheduled_start)} · ${esc(w.customer_name||'Customer')}</h3><div>${esc(w.complaint||'Service call')}</div><div class="muted">${esc([w.city,w.state].filter(Boolean).join(', '))}</div><div>${wazeLink(w,'Navigate with Waze')}</div><button class="primary" data-open="${w.id}">${w.job_type==='delivery'?'Open Delivery':'Open Job'}</button></article>`).join('')||'<div class="card"><b>No jobs shown.</b><p class="muted">Use “Show all techs” if you need to see the full dispatch schedule.</p></div>';$$('[data-open]').forEach(b=>b.onclick=()=>openJob(b.dataset.open))}
